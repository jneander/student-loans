...
function createFilePicker (options) {
  loadFilePicker().then(() => {
    ...
    const googleUser = GoogleApi.auth2.getAuthInstance().currentUser.get();
    ...
    picker = picker.enableFeature(google.picker.Feature.NAV_HIDDEN)
      ...
      .setCallback((data) => {
        if (data.action === google.picker.Action.PICKED && options.onSelect) {
          console.log(`picked file: ${data.docs[0].id}`);
          options.onSelect(data);
        }
        if (data.action === google.picker.Action.CANCEL && options.onCancel) {
