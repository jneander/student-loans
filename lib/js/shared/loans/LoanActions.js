...
const LoanActions = {
  ...
  deleteLoan (loanId) {
    return function (dispatch, getState) {
      dispatch({ type: Constants.DELETE_LOAN_STARTED, payload: { loanId } });
      LoanApi.deleteLoan(loanId)
        .then(() => {
          dispatch({ type: Constants.DELETE_LOAN_SUCCESS, payload: { loanId } });
        })
        .catch((error) => {
          dispatch({ type: Constants.DELETE_LOAN_FAILURE, payload: { loanId, error } });
        });
    }
  }
};
